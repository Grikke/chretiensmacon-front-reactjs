...
const ChurchSVG = ({name= "", x, y, reverse = false, show = false,coordinates}: {
  ...
  return (<g onClick={() => window?.open(coordinates, '_blank')?.focus()} transform={`translate(${x}, ${y})`} style={{opacity: show ? 1 : 0, pointerEvents: show ? 'initial' : 'none'}} className="church-svg">
